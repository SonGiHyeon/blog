...
const BlockChainInfo = ({ activeContent }: { activeContent: string }) => {
    ...
    const renderContent = () => {
        switch (activeContent) {
                    ...
                    <div className="container">
                        ...
                        <div className='section'>
                            ...
                            <ul>
                                ...
                                <li><strong>2. 데이터의 해시 값 생성</strong> {arrow} 암호학적 해시 함수(Hash Function) 적용 {arrow} 해시 함수는 데이터를 고정된 길이의 ‘해시 값’으로 변환한다.
                                </li>
                                <ul>
                                    <li>입력 데이터가 어떤 크기이든 상관없이 고정된 크기의 해시 값으로 압축된다.</li>
                                    <li>데이터가 조금이라도 변경되면 완전히 다른 해시 값이 생성된다.</li>
                                </ul>
                                <li><strong>3. 해시 값을 타임스탬프 기관(Timestamp Authority, TSA)에 제출</strong> {arrow} 타임스탬프 기관은 논문 상에서 “신뢰할 수 있는 제 3자가 해시를
                                ...
                                <li><strong>4. 타임 스탬프와 해시 값 반환</strong> {arrow} TSA는 사용자가 제출한 해시 값에 타임스탬프와 서명을 추가한 결과를 사용자에게 반환한다.</li>
                                <li><strong>5. 데이터 검증 방법</strong> {arrow} 데이터의 타임스탬프의 검증 방법: 원본 데이터의 해시 값을 다시 계산(SHA-256으로 자신의 데이터를 해싱) {arrow} 계산한 해시
                                    값과
                                ...
                                <li><strong>6. 타임스탬프의 블록체인 활용</strong> {arrow} 각 해시 값은 이전 해시와 연결되어서 체인을 형성한다.</li>
                                <ul>
                                    <li>각 타임스탬프가 이전 타임스탬프의 해시 값을 포함하도록 설계 {arrow} 이렇게 하면 시간 순서가 기록되며, 이전 데이터의 무결성을 보장</li>
                                </ul>
                            </ul>
                        </div>
                        ...
                        <div className='section'>
                            <h3>해시 생성 방법: 해시 함수(Hash Function)이용</h3>
                            <ul>
                        ...
                        <div className='section'>
                            ...
                            <ul>
                                <li><strong>(a) 메시지 스케줄 생성</strong> {arrow} 각 512 비트 블록은 32비트 단위로 나뉘며, 한 블록 당 16개의 워드(word)로 시작한다.</li>
                                <ul>
                                    <li>워드: 비트 단위(정보 표현)</li>
                                    <li>추가적으로 48개의 워드 생성 {arrow} 총 64개의 워드가 생성된다.</li>
                                    <li>새로운 워드는 이전 워드의 XOR 및 시프트 연산으로 생성된다.</li>
                                </ul>
                                <li><strong>(b) 해시 압축 함수(라운드 연산)</strong> {arrow} 초기 해시 값과 64개의 워드 그리고 라운드 상수(64개의 고정된 32비트 상수 값)이 Round
                            ...
                            <h3>최종 출력</h3>
                            <ul>
                                <li><strong>최종 해시 값 갱신</strong> {arrow} 라운드가 끝난 후 a, b, c, d, e, f, g, h 값을 초기 해시 값에 추가한다.</li>
                        ...
                        <div className='section'>
                            ...
                            <p>{arrow} 위의 데이터를 결합하여 SHA-256 해시 함수를 두 번 적용해서 블록 해시 값을 생성한다.</p>
                            <p>{arrow} Block Hash = SHA256(SHA256(Block Header))</p>
                        ...
                        <div className='section'>
                            ...
                            <p>{arrow} 위의 입력 값들 중에서 논스, 타임 스탬프, 머클 루트가 해시 퍼즐을 어렵게 만드는 요인들이다.</p>
                        </div>
                        ...
                        <h2>대칭키 암호화 & 비대칭키 암호화</h2>
                        <div className='section'>

                            <h3>대칭키 암호화</h3>
                            <ul>
                                <li>암호화 및 복호화를 할 때 동일한 키를 사용하는 방식 {arrow} 여러 사용자 사이에 공유된 단 하나의 키를 기반으로 동작한다.</li>
                            </ul>

                            <h3>대칭키 암호 방식 동작 원리</h3>
                            <ul>
                    ...
                    <div className="container">
                        ...
                        <div className='section'>
                            <ul>
                                ...
                                <li>입력(Input) {arrow} 입력은 트랜잭션이 사용하는 ‘이전 트랜잭션의 출력(UTXO: Unspent Transaction Output)’을 참조한다. 입력은 송신자의 ‘서명’이
                                    포함된
                                    정보다.
                                    <ul>
                                        ...
                                        <li>출력 인덱스(Output Index)
                                            <ul>
                                                <li>참조하는 트랜잭션에서 어떤 출력(0으로 시작하는 인덱스)을 사용하는지 나타낸다. {arrow} 지출할 UTXO</li>
                                                <li>크기: 4바이트</li>
                                            </ul>
                                        </li>
                                ...
                                <li>출력(Output) {arrow} 수신자에게 전달되는 비트코인 정보, 한 트랜잭션은 ‘여러 개의 출력’을 가질 수 있다.
                                    <ul>
                                        <li>출력 금액(Value)
                                            <ul>
                                ...
                                <li>잠금 시간(Lock Time {arrow} 미래의 특정 시점에 거래를 실행할 수 있도록 예약하는 기능
                                    <ul>
            ...
            // default:
            //     return <div>블록체인 관련 내용을 선택해 주세요.</div>;
        }
    };
